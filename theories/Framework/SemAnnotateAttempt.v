...
From Framework Require Import Util W0 ANF0 ANF1 ANF Label Checking Reify RelComp.
...
(* This file is used to illustrate we cannot establish an equivalence between the top-level abstract relation from the Annotate functor and the top-level composed relation. *)
(* This explains why the Annotate functor is too abstract for semantic analyses and why we need [SemAnnotate.v]. *)

(* Top-level Logical Relations *)
(* These are essentially the unpacked Annotate functor definitions. *)
...
(* This is not provable.
   The top-level environment will get into the way.
    Note here we have to construct an annotated ρ on the spot. But we cannot. *)
Lemma V_V_top :
  forall i v1 v2,
    exposed v2 ->
...
Definition T_top := Cross L.trans_correct_top
...
Proof.
  unfold T_top, Cross, trans_correct_top, L.trans_correct_top, C.trans_correct_top, R.trans_correct_top.
  intros.
  destruct H as [e3 [HL [e3' [HC HR]]]].
  destruct HL as [HS1 HL].
  destruct HC as [HS2 [HW HC]].
  destruct HR as [HS3 HR].
