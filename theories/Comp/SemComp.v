...
From SemAnnotate Require Import Comp.
...
Module C := SemAnnotate.Comp.

Section Comp_n.

  Definition Top_n n m := Cross (Cross (C0.Top_n n) C.trans_correct_top) (C1.Top_n m).

  Definition V_n n m := Cross (Cross (C0.V_n n) C.V) (C1.V_n m).
  ...
  Proof.
    ...
    eapply C.V_wf_val_r in HA; eauto.
  ...
  Proof.
    ...
    edestruct C.R_res_inv_l as [v2' [Heqv2' HV]]; eauto; subst.
    eexists; split; eauto.
  ...
  Proof.
    ...
    apply C.trans_correct_top_subset in HA.
...
Section Adequacy.
  ...
  Lemma Top_n_adequacy n m e1 e2:
    Top_n n m e1 e2 ->
    forall ρ1 ρ2,
      wf_env ρ2 ->
  ...
  Proof.
    ...
    edestruct (C0.Top_n_adequacy _ _ _ HC0) with (ρ2 := ρ1') as [j1' [r1' [Hstep1' HRn0]]]; eauto.

    assert (wf_env ρ2').
    {
      specialize (HAG 0).
      eapply C.G_wf_env_r; eauto.
    }
    ...
    - intros.
      ...
      eapply C.trans_correct_subset; eauto.
    - edestruct (C1.Top_n_adequacy _ _ _ HC1) with (ρ2 := ρ2) as [j2 [r2 [Hstep2 HRn1]]]; eauto.
      + eapply C1.G_n_subset; eauto.
        * eapply C.trans_correct_subset in HA; eauto.
          eapply Included_trans; eauto.
          eapply C0.Top_n_subset; eauto.
...
Section Linking.
  ...
  Proof.
    ...
    assert (Hf4' :  ~ Ensembles.In var (C.AS.occurs_free e4') f).
    {
      eapply C0.Top_n_subset in HC0'; eauto.
    }

    eapply (C.preserves_linking f w x e4 e4' e3 e3') in HA1; eauto.
