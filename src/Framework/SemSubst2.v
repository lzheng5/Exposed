...
Module VTransM <: Exposed.VTrans LM.
  ...
  Definition V_trans
    ...
    | Vfun f_wrap ρ1 xs1 e1 =>
        forall f_work ρ,
          let wrapper := (wrap bs xs1 (* REVISIT *)  f_work w_work f_wrap w_wrap) in
          (~ f_work \in bound_var wrapper) ->
...
Proof.
  intros Hwrapper Hf_work Hf_work' Hw_work Hw_work1 Hinfo Hinfo1 i.
  edestruct bound_vars_wrap_inv as [Hf_work1 Hf_work2]; eauto.
...
Proof.
  ...
  unfold V_trans.
  repeat (split; auto); intros.
  edestruct (bstep_fuel_wrap tinfo xs f_work0 w_work f_wrap w_wrap (Tag w_work v_work) ρ0) as [k' [ρ'' [xs' [e' Hstep]]]]; eauto.
  eexists; eexists; eexists; eexists; split; eauto; intros.

  assert (Hrefl : match i with
                  | 0 =>
                      V_refl0 (Vfun f_wrap ρ' xs e) (Vfun f_wrap ρ'' xs' e')
                  | S i0 =>
                      V_refl (fun j => V (i0 - (i0 - j))) (fun j => E false (i0 - (i0 - j))) i0 w_work v_work v_work' ->
                      V_refl (fun j => V (i0 - (i0 - j))) (fun j => E false (i0 - (i0 - j))) i0 w_wrap (Vfun f_wrap ρ' xs e) (Vfun f_wrap ρ'' xs' e')
                  end).
  {
    eapply V_wrapper_refl; eauto.
