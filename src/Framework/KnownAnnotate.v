...
Section Join.
  ...
  Lemma fun_compat_top_join K1 K2 e e' k k' f xs :
    known_map_inv K1 ->
    K1 ! f = None ->
    Disjoint _ (FromList xs) (Dom_map K1) ->

    known_map_inv K2 ->
    K2 ! f = None ->
    Disjoint _ (FromList xs) (Dom_map K2) ->

    Disjoint _ (Dom_map K1) (Dom_map K2) ->

    trans_correct_top K1 e e' ->
    trans_correct_top K2 k k' ->
    trans_correct_top (join K1 K2) (A0.Efun f xs e k) (Efun f w0 xs e' k').
  Proof.
    unfold trans_correct_top, E, E'.
    intros HK1 HK1f HK1xs HK2 HK2f HK2xs HD; intros.
    assert (Hw0 : w0 \in Exposed) by (apply w0_exposed).
  ...
  Lemma G_top_join_r K1 K2 i Γ1 ρ1 Γ2 ρ2 :
    Disjoint _ (Dom_map K1) (Dom_map K2) ->
    G_top (join K1 K2) i Γ1 ρ1 Γ2 ρ2 ->
    G_top K2 i Γ1 ρ1 Γ2 ρ2.
  Proof.
    unfold G_top.
    intros.
    destruct H0 as [Hρ [HS HG]].
    repeat (split; auto); intros.
    edestruct HG as [v1 [v2 [Heqv1 [Heqv2 [Hb HV]]]]]; eauto.
    assert (binding_inv_top K2 x v2) by admit.
    eexists; eexists; repeat (split; eauto).
    admit.
  Admitted.

  Lemma join_get_Some_r K1 K2 x w:
    Disjoint _ (Dom_map K1) (Dom_map K2) ->
    K2 ! x = Some w ->
    (join K1 K2) ! x = Some w.
  Proof.
    intros.
    pose proof H as H'.
    apply join_sound in H.
    unfold join_spec in *.
    destruct (H x) as [H1 [H2 H3]]; clear H.
    eapply H2; eauto.
  ...
  Lemma V_join_r K1 K2 :
    Disjoint _ (Dom_map K1) (Dom_map K2) ->
    forall i v1 v2,
      V (join K1 K2) i v1 v2 ->
      V K2 i v1 v2.
  Proof.
    intros HD i.
    induction i using lt_wf_rec; intros.
    ...
    - destruct v2; destruct v1.
      + destruct v; simpl in *.
        destruct H0 as [Hv [Heqv [Hlen HV]]]; subst.
        repeat (split; auto).
        destruct (K2 ! v) eqn:HKv.
        * erewrite join_get_Some_r in HV; eauto.
        *
  Admitted.
