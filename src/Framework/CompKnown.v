...
Module M.
  ...
  Section Comp_n.
    ...
    Definition Top_n n m := Cross (Cross (C0.Top_n n) (fun e1 e2 => AM.trans_correct_top (AM.analyze e1) e1 e2)) (C1.Top_n m).

    Definition V_n n m := Cross (Cross (C0.V_n n) (fun v1 v2 => exists K, forall k, AM.V K k v1 v2)) (C1.V_n m).

    Definition R_n n m := Cross (Cross (C0.R_n n) (fun v1 v2 => exists K, forall k, AM.R K k v1 v2)) (C1.R_n m).

    Definition G_n n m Γ1 Γ2 := Cross (Cross (C0.G_n n Γ1 Γ2) (fun ρ1 ρ2 => forall K k, AM.G_top K k Γ1 ρ1 Γ2 ρ2)) (C1.G_n m Γ1 Γ2).
    ...
    Proof.
      ...
      apply (AM.trans_correct_top_subset (AM.analyze e1')) in HA.
  ...
  Section Adequacy.
    ...
    Proof.
      ...
      assert (wf_env ρ2').
      {
        specialize (HAG (M.empty _) 0).
        eapply AM.G_top_wf_env_r; eauto.
      }

      destruct (AM.analyze_sound e1') as [_ [HK _]].
      edestruct (AM.adequacy _ _ _ HK HA) with (ρ2 := ρ2') as [j2' [r2' [Hstep2' HAR]]]; eauto.
      - intros.
        eapply AM.G_top_subset; eauto.
        eapply C0.Top_n_subset; eauto.
  ...
  Section Linking.
    ...
    Proof.
      ...
      remember (AM.analyze e4) as K1.
      remember (AM.analyze e4') as K2.
      remember (AM.join K1 K2) as K3.

      assert (HK3A1 : AM.trans_correct_top K3 e4 e3) by admit.
      assert (HK3A1' : AM.trans_correct_top K3 e4' e3') by admit.

      eapply (AM.preserves_linking K3 f w x e4 e3 e4' e3') in HK3A1; eauto.
