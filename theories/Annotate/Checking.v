...
Lemma G_top_wf_cenv_r i Γ1 ρ1 ρ2 :
  G_top i Γ1 ρ1 ρ2 ->
  wf_cenv ρ2.
Proof. unfold G_top. intros; tauto. Qed.

Lemma G_top_refine_env i Γ1 ρ1 ρ2 :
  G_top i Γ1 ρ1 ρ2 ->
  refine_env Γ1 ρ1 ρ2.
Proof. unfold G_top. intros; tauto. Qed.
...
Proof.
  ...
  destruct HG as [Hwf1 [Hwf2 [Href HG]]].
  split.
  eapply wf_env_set; eauto.
  eapply V_top_wf_val_l; eauto.
...
Proof.
  intros i r1 r2 r3 Hwf1 Hwf2 Heqv.
  unfold R_top, R'.
  inv Heqv.
  - (* res_eqv OOT OOT *)
    destruct r3; tauto.
  - (* res_eqv (Res v1) (Res v2) *)
    destruct r3; try tauto.
    inv Hwf1. inv Hwf2.
...
Proof.
  ...
  - (* BStep_letapp_OOT *)
    match goal with
    | [ Hb : bstep_fuel _ _ _ OOT |- _ ] => rename Hb into Hbody
    end.
    rename f' into f.
    rename ρ' into ρ1.
    rename e into e1.
...
Proof.
  ...
  - intros i ρ1 ρ2 HG.
    ...
    assert (Hwfr2'' : wf_res r2'').
    {
      ...
      destruct r2''; destruct r2'''; try contradiction; auto.
    ...
    assert (Hwfsetenv :
      wf_env (f |: (x |: occurs_free (Efun f l0 [] e (Eletapp x f l0 [] e2))))
             (M.set f (Tag l0 (Vfun f ρ1 [] e)) (M.set x v ρ1))).
    {
      eapply wf_env_set; [|exact Hwfclos].
