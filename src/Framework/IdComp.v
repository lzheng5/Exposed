...
From Framework Require Import Util RelComp ANF Id.
...
Section IdComp.
  ...
  Definition Top_n := Comp trans_correct_top.
...
Section Adequacy.
  ...
  Proof.
    ...
    - inv H2.
      ...
      eapply G_n_subset with (Γ3 := occurs_free c3); eauto.
    ...
    - destruct v2.
      ...
      + destruct (H0 1) as [Hv1 [Hv2 [Hw [Hc HV']]]]; subst;
        ...
        assert (HV' : forall i, V i v y /\ V i (Tag w1 (Vconstr c0 vs)) (Tag w1 (Vconstr c0 l'))).
        {
          ...
          specialize (H0 (S i)).
          destruct i; simpl in *;
            destruct H0 as [_ [_ [_ [_ HFV]]]]; subst;
            inv HFV.
          - inv Hv1; inv Hv2.
            inv H4; inv H7.
            repeat (split; auto).
...
Section Linking.
   ...
   * Thus, we need to show the compat lemmas for fun and letapp with [trans_correct_top] *)
  Lemma trans_correct_preserves_linking f w x e1 e2 e1' e2':
    (w \in Exposed) ->
    trans_correct (Empty_set var) e1 e2 ->
    trans_correct [set x] e1' e2' ->
    trans_correct (Empty_set var) (link f w x e1 e1') (link f w x e2 e2').
  Proof.
    unfold link.
    intros Hw He He'.
    eapply fun_compat; eauto.
    - unfold trans_correct in *.
      intros.
      eapply He; eauto.
      eapply G_empty; eauto.
  ...
  Lemma trans_correct_trans_correct_top e1 e2 :
    occurs_free e2 \subset occurs_free e1 ->
    trans_correct (occurs_free e1) e1 e2 ->
  ...
  Proof.
    ...
    - destruct vs1; try discriminate.
      ...
      inv H; inv H0; inv H1; inv H2; inv H3.
      destruct HG as [Hr1 [Hr2 HG]].
  ...
  Lemma fun_compat_top e e' k k' f w xs :
    ...
    trans_correct_top (Efun f w xs e k) (Efun f w xs e' k').
  Proof.
  ...
  Lemma letapp_compat_top k k' w xs x f :
    (w \in Exposed) ->
    trans_correct_top k k' ->
    trans_correct_top (Eletapp x f w xs k) (Eletapp x f w xs k').
  Proof.
    unfold trans_correct_top, E, E'.
    intros.
    destruct H0.
  ...
  (* Linking Preservation for [trans_correct_top] *)
  Theorem trans_correct_top_preserves_linking f w x e1 e2 e1' e2':
    (w \in Exposed) ->
    trans_correct_top e1 e2 ->
    trans_correct_top e1' e2' ->
    trans_correct_top (link f w x e1 e1') (link f w x e2 e2').
  Proof.
    unfold link.
    intros.
  ...
  Proof.
    ...
    - assert (He1' : trans_correct_top e1' e1') by (eapply refl_trans_correct_top; eauto).
      econstructor; eauto.
      eapply trans_correct_top_preserves_linking; eauto.
      eapply IHHrel; eauto.
  Qed.
